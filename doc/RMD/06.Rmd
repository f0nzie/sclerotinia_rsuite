...
vp11     <- varpart(dat11cc.bruvo, ~Year + Region + Host + MCG, data = ENV11, 
                    comm = dat11raw, add = TRUE)
plot(vp11, Xnames = c("Full Model", "No Model"))
try(vp16     <- varpart(dat16cc.bruvo, ~Year + Region + Host, data = ENV16, 
                        comm = dat16raw, add = TRUE))
```

```{r}
vp11 <- varpart(dat11cc.bruvo, ~Year, ~Region, ~Host, ~MCG, data = ENV11, 
                add = TRUE)
vp16 <- varpart(dat16cc.bruvo, ~Year, ~Region, ~Host, ~MCG, data = ENV16, 
                add = TRUE)
