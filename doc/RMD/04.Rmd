...
```{r}
dir.create(file.path(PROJHOME, "results", "figures", "publication"), 
...
plot_mlg_graph <- function(g, glayout = NULL, label = TRUE, seed = 2017-06-28){
  ...
  g2          <- g
  V(g2)$label <- ifelse(!is.na(V(g2)$name), 
                        sprintf("%s [(%f/%f)]", V(g2)$name, shared_mlg, 
                                V(g2)$size), NA)
...
gg16 <- plot_mlg_graph(graph16loc$total, alt_layout) 
#+ labs(list(subtitle = "(16 loci)"))
gg16
ggsave(filename = file.path(PROJHOME, 
                            "results/figures/publication/FigureS1Z.pdf"), 
       width = 88, height = (6.25/7)*88, units = "mm", scale = 2)
```
...
cowplot::ggsave(plot = gg16cp, 
                file = file.path(PROJHOME, "results", "figures", "publication", 
                                 "mlg-16.pdf"),
                width = 88,
                height = (6.25/7)*88,
                units = "mm",
...
gg11
ggsave(filename = file.path(PROJHOME, "results/figures/publication/Figure3Z.pdf"), width = 88, height = (6.25/7)*88, units = "mm", scale = 2)
```
...
```{r, saving_data}
save(graph16loc, graph11loc, alt_layout, 
     file = file.path(PROJHOME, "data/mlg-crosspop-graph.rda"))
```
