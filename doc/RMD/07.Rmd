...
ptab %>%
  ...
  write_csv(file.path(PROJHOME, "results", "tables", 
                      "genotype_diversity_table_region.csv"), 
            col_names = TRUE) %>%
  mutate(Ia = case_when(
    !is.finite(p.Ia) ~ "-",
...
cowplot::ggsave(filename = file.path(PROJHOME, "results", "figures", 
                                      "publication/DAPC.pdf"),
       width = 13,
       height = 6
      )
```
